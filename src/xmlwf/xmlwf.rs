...
unsafe extern "C" fn startElementNS(
    ...
    while !(*atts).is_null() {
        ...
        putc(' ' as i32, fp);
        if !sep.is_null() {
            ::rexpat::stdlib::fprintf(fp, b"n%d:\x00".as_ptr() as *const c_char, nsi);
        ...
        if !sep.is_null() {
            let fresh2 = nsi;
            nsi = nsi + 1;
            ::rexpat::stdlib::fprintf(fp, b" xmlns:n%d\x00".as_ptr() as *const c_char, fresh2);
            attributeValue(fp, name);
        }
        atts = atts.offset(1)
    }
...
unsafe extern "C" fn metaLocation(mut parser: XML_Parser) {
    ...
    if !uri.is_null() {
        ::rexpat::stdlib::fprintf(fp, b" uri=\"%s\"\x00".as_ptr() as *const c_char, uri);
    }
    ::rexpat::stdlib::fprintf(
        fp,
        ...
        ::rexpat::lib::xmlparse::XML_GetCurrentLineNumber(parser),
        ::rexpat::lib::xmlparse::XML_GetCurrentColumnNumber(parser),
    );
}
...
unsafe extern "C" fn metaStartNamespaceDecl(
    ...
    if !prefix.is_null() {
        ::rexpat::stdlib::fprintf(fp, b" prefix=\"%s\"\x00".as_ptr() as *const c_char, prefix);
    }
    if !uri.is_null() {
...
unsafe fn main_0(mut argc: c_int, mut argv: *mut *mut XML_Char) -> c_int {
    ...
    while i < argc {
        ...
        let mut parser: XML_Parser = 0 as *mut ::rexpat::expat_h::XML_ParserStruct;
        if useNamespaces != 0 {
        ...
        ::rexpat::lib::xmlparse::XML_ParserFree(parser);
        if result == 0 {
            exit(2i32);
        }
        i += 1
